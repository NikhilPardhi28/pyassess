...
ADMIN_USER = 'admin'
ADMIN_PASS_HASH = hashlib.sha256('Admin@PyTest2024'.encode()).hexdigest()
...
def test_page(token):
    ...
    return render_template('test.html', token=token, candidate_name=s.get('name','Candidate'))
...
def get_questions(token):
    ...
    # Don't send answers to candidate
    safe_qs = []
    for q in questions:
...
def eject_candidate():
    data = request.json
    token = data.get('token')
    reason = data.get('reason', 'Voice/noise detected')
    sessions = load_json(SESSIONS_FILE, {})
    if token in sessions:
        sessions[token]['status'] = 'ejected'
        sessions[token]['eject_reason'] = reason
...
def update_code_review():
    ...
    if result_id in results and qid in results[result_id].get('code_answers', {}):
        results[result_id]['code_answers'][qid]['admin_score'] = score
        results[result_id]['code_answers'][qid]['admin_note'] = note
        results[result_id]['code_answers'][qid]['admin_reviewed'] = True
        ...
        r['final_score'] = r['mcq_score'] + code_total
        save_json(RESULTS_FILE, results)
